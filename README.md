...
Source and destination provider can also be the same — e.g. moving files into a different bucket
on the same DigitalOcean/Wasabi account. `destBucket` is then required, and must differ from every
source item's bucket, since otherwise each item would target its own source bucket/key and the
"copy" would silently no-op onto itself.
...
The reverse of the archive-and-delete flow's copy-to-archive step (see `archive_and_delete_objects`
above), for pulling a file back out of `scola-school-archives` (or whatever `ARCHIVE_S3_BUCKET` is
set to) to where it used to live. Paste one or more archive URLs or bare paths — one per line —
into the UI; `POST /storage/parse-archive-refs` resolves each against every configured provider's
endpoint the same way a DB path is during reconciliation (`_split_object_reference`), or falls
back to treating a bare path's first `/`-separated segment as the bucket. Either way it comes out
as `{bucket: "<archive bucket>", key: "<original bucket>/<original key>"}` — e.g.
`https://sgp1.digitaloceanspaces.com/scola-school-archives/scola-rosetta/s3_ckf_files/files/foo.mp3`
becomes `bucket="scola-school-archives"`, `key="scola-rosetta/s3_ckf_files/files/foo.mp3"`.

As an alternative to pasting lines by hand, `POST /storage/parse-bucket-objects-report` accepts a
"List bucket objects" report (`.xlsx`) generated by listing the archive bucket itself — trim it
down to just the rows wanted first if not everything in it should be restored. Its `Key` column is
already in the `<original bucket>/<original key>` archive form, and the bucket comes from the
report's own `Summary` sheet, so no URL parsing is needed for this path.

`POST /storage/restore-from-archive` / `/restore-from-archive/stream` then copy each item back to
`bucket=key.split("/", 1)[0]`, `key=key.split("/", 1)[1]` — i.e. the archive key's leading segment
becomes the destination bucket, and the rest becomes the destination key, which is exactly the
convention `archive_and_delete_objects` wrote it with in the first place. Same non-destructive
stance as `/copy`: the archive copy itself is never touched, so this can be re-run freely. Same
audit trail (`reports/.restores/`) and `.xlsx` report (`Restored` / `Skipped` / `Failed` sheet)
pattern as the plain copy flow, just with the destination URL built from the resolved
`destBucket`/`destKey` instead of reusing the source key.
...
| POST | `/storage/parse-archive-refs` | Parse pasted archive URLs/paths into `{bucket, key}` pairs for restore-from-archive |
| POST | `/storage/parse-bucket-objects-report` | Parse an uploaded "List bucket objects" report of the archive bucket into `{bucket, key}` pairs for restore-from-archive |
| POST | `/storage/restore-from-archive` | Restore archived objects to their original bucket/key (blocking, single response) |
| POST | `/storage/restore-from-archive/stream` | Restore archived objects to their original bucket/key (SSE, live progress) |
